...
Code.prototype.setPubsub = function(pubsub){
	...
	if (self.pubsub.$ready){ //If it's not ready, then only instrumentation is possible
		self.pubsub.subscribe(self.id+'/snapshots', function onNewMessage(snapshot){
	  		console.log("<SMARTCODE> ["+self.id+"] : New Snapshot received from <CODE> ["+self.id+"]");
...
Code.prototype.saveToFile = function(filename){
	...
	console.log("<SMARTCODE> ["+this.id+"] : Created new Code at "+filename);
...
Code.prototype.saveLastSnapshot = function(filename){
	...
	console.log("<SMARTCODE> ["+this.id+"] : Saved Snapshot at "+filename);
};
Code.prototype.run = function(){
	console.log("<SMARTCODE> ["+this.id+"] : Executing Code ["+this.id+"]");
	this.code();
};
...
//	console.log("<SMARTCODE> ["+this.id+"] : Destroyed!");
};
...
Code.prototype._convertToCode = function(){
	...
	console.log("<SMARTCODE> ["+this.id+"] : Instrumentation time - "+(this.MEASUREMENTS.end - this.MEASUREMENTS.start)+" ms");
		if (DEBUG_MODE){
...
//					console.log("Required '"+declaration.init.arguments[0].value+"'");
...
Code.prototype._processCallExpression = function(node, parentScope){
	...
	if (node.callee.type === 'Identifier' && (['setImmediate', 'setTimeout', 'setInterval', 'clearImmediate', 'clearTimeout', 'clearInterval'].indexOf(node.callee.name) > -1)){
	...
	return { 'scope': parentScope, 'injection': null, 'replace': replace };
};
...
//	}
	...
	restoredCode.sourcePath = 'inputs/'+program.codeId+"-"+program.migrationCount+".js";
