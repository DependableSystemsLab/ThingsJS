...
var NATIVE_CONSTRUCTORS = [ 'Date', 'Promise' ];
...
var THINGSJS_OBJECT_HANDLERS = {
	'Pubsub': {
		...
		deserialize: function(scope, data, functionTable){
			...
			return new Pubsub(data.id, data.pubsub_url, handlers);
		}
	}
}
...
//		console.log(variable.constructor);
		...
		if (variable.constructor.fnid){
			...
			serialized = "new things_js.Pubsub('" + variable.id + "', '"+variable.pubsub_url+"', "+Scope._processVariable(scope, variable.handlers, functionTable, childScopesReady)+", true)";
		}
		return serialized;
	}
	else {
