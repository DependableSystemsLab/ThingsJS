...
// Replacement for console.log (intercept log calls and publish to topic for monitoring)
...
//	console.log(data);
	// console.log(functionTable[data.fnid].code);
...
Scope._processScope = function(scope, functionTable){
	...
	for (var varName in scope.requires){
		if (scope.requires[varName] === 'things-js/lib/mods/fs'){
			code += "var "+varName+" = require('"+scope.requires[varName]+"')(Σ_global, true);\n";
		}
		else {
...
Scope.generateProgram = function(snapshot){
	...
	var program = header + ".then(function(){\n" + closures + timers + "\n}, function(err){ console.log('Failed to connect to Pubsub at ${snapshot.pubsub_url}') });\n" + functionTable;
