...
function Engine(dispatcher, id, meta){
	...
	this.pubsub.subscribe(this.dispatcher.id+'/'+this.id, function(message, topic){
		if (message.reply_id in self._requests){
			...
			console.log(chalk.red('[Dispatcher:'+self.dispatcher.id+'] Received unexpected message'));
		}
	});
...
Program.prototype.kill = function(code_name, instance_id){
...
// Dispatcher.prototype.killCode = function(engine_id, code_name, instance_id){
// 	return this.engines[engine_id].killCode(code_name, instance_id);
// };
Dispatcher.prototype.killCode = function(code_name, instance_id){
	return this.programs[instance_id].kill(code_name, instance_id);
};
