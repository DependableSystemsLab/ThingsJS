...
function CodeEngine(config, options){
	...
	if (this.options.enable_status_led){
		var led = new StatusLED(4);
		this.on('status-change', function(status){
			if (status === 'busy') led.on();
			else led.off();
		})
	}
}
...
CodeEngine.prototype.reportStatus = function(eventName){
	...
	if (curStatus !== this.status) this.emit('status-change', this.status);
...
function __dummyCallback(){};
function StatusLED(pin){
	...
	this.on = function(){
		timer = setInterval(function(){
			value = value ? 0 : 1;
			led.write(value, __dummyCallback);
		}, 250);
	};
	this.off = function(){
		clearInterval(timer);
