...
function Pubsub(url, options){
	...
	this.clientSub.on('message', function(rawTopic, rawMessage, packet){
		...
		var message = rawMessage.slice(lineBreak+1).toString();
		message = Buffer.from(message, "base64");
		//var message = Buffer.from(rawMessage.slice(lineBreak+1), "base64");
...
Pubsub.prototype.buildCommand = function(clientId, command, topic, msg) {
	...
	if (typeof(msg) !== 'undefined') {
		var msgString = msg.toString("base64");
		totalSize += msgString.length;
	}
