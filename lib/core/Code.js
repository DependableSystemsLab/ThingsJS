...
Code.prototype.reportStatus = function(){
	var self = this;
	Object.values(self.processes).forEach(function(proc){
...
Code.prototype.run = function(options){
	...
	return new Promise(function(resolve, reject){
		...
		forked.on('message', function(message){
			...
			if (message.reply_id in forked._requests){
				forked._requests[message.reply_id].resolve(message.payload);
				delete forked._requests[message.reply_id];
			}
			...
			else if (message.ctrl && message.ctrl === 'log-event'){
				self.processes[forked.instance_id].logs.push(message.payload);
				console.log(chalk.yellow('[child:'+forked.pid+'] '+self.name+': ')+message.payload.timestamp+' : '+message.payload.event);
			}
...
Code.prototype.ipcSend = function(instance_id, ctrl, kwargs){
	...
	return new Promise(function(resolve, reject){
		...
		if (instance && instance.process.connected){
			...
			instance.process.send({ request_id: req_id, ctrl: ctrl, kwargs: kwargs });
			setTimeout(function(){
...
Scope.prototype.addFunction = function(func, protects, uid){
	...
	func[PROPERTY_PREFIX+'wrapped'] = wrapped;
...
Code.bootstrap = function(pubsub_url, code_uri){
	...
	if (process.connected){
		...
		// root_scope.reportResource = function(){
		...
		// root_scope._resumeReport = function(){
		// 	(function next(){
		// 		root_scope.reportResource();
		// 		root_scope.report_timer = setTimeout(next, Math.round(1000*Math.random() + 500))
		// 	})();
		// }
		// root_scope._pauseReport = function(){
		// 	if (root_scope.report_timer) clearTimeout(root_scope.report_timer);
		// }

		// // Report resource usage to master process (TODO: make this optional)
		// setInterval(root_scope.reportResource, 1000);
