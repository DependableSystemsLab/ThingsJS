...
// 	obj[PROPERTY_PREFIX+'uid'] = uid || helpers.randKey();
// 	obj[PROPERTY_PREFIX+'owner'] = this;
// 	this.objs[obj[PROPERTY_PREFIX+'uid']] = obj;
// 	return obj;
// }
...
function captureObject(timestamp, obj, scope){
	if (obj){
		if (typeof obj === 'function'
			...
			Object.keys(obj.prototype).forEach(function(prop){
				...
				if (prop[0] !== PROPERTY_PREFIX){
					captureObject(timestamp, obj.prototype[prop], scope);
				}
			});
			Object.keys(obj).forEach(function(prop){
				if (prop[0] !== PROPERTY_PREFIX){
					( DEBUG && console.log(helpers.indent(scope.level*4 + 2)+chalk.cyan(prop)+ ':') );
					captureObject(timestamp, obj[prop], scope);
				}
			});
		}
			...
			if (obj[PROPERTY_PREFIX+'scope']) obj[PROPERTY_PREFIX+'scope'].capture(timestamp);
			Object.keys(obj).forEach(function(prop){
				if (prop[0] !== PROPERTY_PREFIX){
					( DEBUG && console.log(helpers.indent(scope.level*4 + 2)+chalk.cyan(prop)+ ':') );
					captureObject(timestamp, obj[prop], scope);
...
Scope.prototype.capture = function(timestamp){
	...
	Object.keys(params).forEach(function(name){
		...
		captureObject(timestamp, params[name], self);
	});
	...
	Object.keys(this.funcs).forEach(function(uid){
		...
		captureObject(timestamp, self.funcs[uid], self);
	});
	...
	Object.keys(refs).forEach(function(name){
		...
		captureObject(timestamp, refs[name], self);
	});
...
RootScope.prototype.snapshot = function(){
	...
	var scope_tree = this.capture(Date.now());
