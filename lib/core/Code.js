...
AstHook.onEnter = {
	...
	CallExpression: function(self){
		...
		if (self.node.callee.type === 'MemberExpression'){
			...
			else if (self.node.callee.object.name in AstHook.NATIVE_KEYWORDS){
				// Do nothing
			}
			else {
	...
	ForInStatement: function(self){
		...
		if (self.node.left.type === 'ExpressionStatement'){
			...
			// console.log(self.node.left.right);
...
Code.prototype.pause = function(){
	var self = this;
	if (self.status === Code.Status.RUNNING){
		return self.ipcSend('PAUSE').then(function(reply){
...
Code.prototype.resume = function(){
	var self = this;
	if (self.status === Code.Status.PAUSED){
		return this.ipcSend('RESUME').then(function(reply){
			self.status = reply.status;
			return self.status;
		})
	}
...
Code.prototype.snapshot = function(pause_after){
	var self = this;
	return this.ipcSend('SNAPSHOT', { pause_after: (pause_after || false) })
		.then(function(reply){
			self.snapshots.push({
				created_at: Date.now(),
...
Code.prototype.getStats = function(){
	var self = this;
...
Scope.prototype.protectAncestry = function(ancestor){
	this.in_use = true;
	// console.log('    protected '+this.uri());
...
Scope.prototype.addFunction = function(func, protects, uid){
	var self = this;
	// console.log(func.name+' protecting '+protects+' now');

	// func[PROPERTY_PREFIX+'protects'] = protects || null;
	...
	func[PROPERTY_PREFIX+'uid'] = uid || func.name+'.'+helpers.randKey(6);
	func[PROPERTY_PREFIX+'uri'] = this.uri()+'/'+func[PROPERTY_PREFIX+'uid'];
	func[PROPERTY_PREFIX+'parent'] = this;
	func[PROPERTY_PREFIX+'protects'] = protects || null;
	this.funcs[func[PROPERTY_PREFIX+'uid']] = func;

	wrapped[PROPERTY_PREFIX+'uri'] = this.uri()+'/'+func[PROPERTY_PREFIX+'uid'];
	wrapped[PROPERTY_PREFIX+'protects'] = protects || null;
	wrapped[PROPERTY_PREFIX+'parent'] = this;
...
Scope.prototype.getSerializable = function(options){
	...
	console.log(chalk.yellow(self.uid+' has params: ')+Object.keys(self.params).join(', '));
...
var ProcessHandler = {
	...
	SNAPSHOT: function(root_scope, kwargs){
		...
		console.log('Snapshot taken in '+safe.meta.time_taken+'ms');
		...
		return { status: status, snapshot: safe };
	},
