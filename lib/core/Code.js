...
makeCode['object'] = function(item){
	...
	if (item.proto_uri){
		return SCOPE_PREFIX+'.restoreObject({'+props.join(',')+'}, "'+item.proto_uri+'", "'+item.scope_uri+'", "'+item.obj_id+'")';
	}
	else {
		return '{'+props.join(',')+'}';
	}
 ...
 */
var LastScopeId = 0;
var LastFunctionId = 0;
function Scope(instance, parent, func, extractor, uid){
	this.parent = parent;
	this.func = func;
	this.uid = uid || func.name+'-'+(LastScopeId++);
...
Scope.prototype.addFunction = function(func, context, uid){
	func[PROPERTY_PREFIX+'uid'] = uid || func.name+'-'+(LastFunctionId++);
...
// Scope.prototype.addObject = function(obj, uid){
// 	uid = uid || helpers.randKey();
// 	obj[PROPERTY_PREFIX+'uid'] = uid;
// 	obj[PROPERTY_PREFIX+'owner'] = this;
// 	this.objs[uid] = obj;
// 	Object.keys(obj).forEach(key => {
// 		if (obj[key] instanceof Pointer) obj[key] = obj[key].link();
// 	});
...
var LastCapturedId = 0;

function captureObject(timestamp, obj, scope, exclude){
	if (obj && !NATIVE_OBJECTS.has(obj) && (typeof obj === 'function' || typeof obj === 'object')){
		if (!exclude){
			exclude = new Map();
		}
		if (exclude.has(obj)) {
			// console.log('Already Captured '+String(obj));
			return { type: 'things-js.object', value: exclude.get(obj) }
		}
		//exclude.set(obj, scope.uri() + '.' + helpers.randKey());
		exclude.set(obj, scope.uri() + '.' + (LastCapturedId++));
		// console.log('capturing '+ String(obj));

		if (typeof obj === 'function'
			&& obj[PROPERTY_PREFIX+'owner'] !== scope
			&& !obj[PROPERTY_PREFIX+'import']){
			( DEBUG && console.log(helpers.indent(scope.level*4)+chalk.yellow("capturing function ")+ obj.name) );
			obj[PROPERTY_PREFIX+'owner'].capture(timestamp);
			Object.keys(obj.prototype).forEach(function(prop){
				// ( DEBUG && console.log(helpers.indent(scope.level*4)+chalk.cyan(prop)+ ':') );
				if (prop[0] !== PROPERTY_PREFIX){
					captureObject(timestamp, obj.prototype[prop], scope, exclude);
				}
			});
			Object.keys(obj).forEach(function(prop){
				if (prop[0] !== PROPERTY_PREFIX){
					( DEBUG && console.log(helpers.indent(scope.level*4 + 2)+chalk.cyan(prop)+ ':') );
					captureObject(timestamp, obj[prop], scope, exclude);
				}
			});
		}
		else if (typeof obj === 'object'
				&& !obj[PROPERTY_PREFIX+'import']){
			( DEBUG && console.log( helpers.indent(scope.level*4)+chalk.yellow("capturing object ")+ '{ '+Object.keys(obj).join(', ')+' }' ) );
			if (obj[PROPERTY_PREFIX+'scope']){
				// console.log(obj[PROPERTY_PREFIX+'scope'].uid);
				obj[PROPERTY_PREFIX+'scope'].capture(timestamp);
			}
			Object.keys(obj).forEach(function(prop){
				// console.log(prop);
				if (prop[0] !== PROPERTY_PREFIX){
			...
			( DEBUG && console.log( helpers.indent(scope.level*4)+chalk.yellow("skipping object ")+ '('+(typeof obj)+') '+String(obj) ) );
		}
	}
}

Scope.prototype.capture = function(timestamp){
	if (this.CAPTURING >= timestamp){
		( DEBUG && console.log(helpers.indent(this.level*4 + 2)+chalk.red("Capture in progress ")+this.uri()) );
		return;
	}
	this.CAPTURING = timestamp;
	( DEBUG && console.log(helpers.indent(this.level*4)+chalk.yellow("Capturing ")+this.uri()) );
	var capturedObjects = new Map();
	var serializedObjects = new Map();
	...
	var extract = this.extractor();
	var params = extract[0];
	var refs = extract[1];

	Object.keys(params).forEach(function(name){
		self.snap.params[name] = makeSerializable(params[name], self, serializedObjects);

		captureObject(timestamp, params[name], self, capturedObjects);
	});
	...
	Object.keys(this.funcs).forEach(function(uid){
		...
		getAllPrototypeProperties(self.funcs[uid].prototype)
			.forEach(function(prop){
				if (prop[0] !== PROPERTY_PREFIX){
					self.snap.funcs[uid].proto[prop] = makeSerializable(self.funcs[uid].prototype[prop], self, serializedObjects);
				}
			});
		Object.keys(self.funcs[uid])
			.forEach(function(prop){
				if (prop[0] !== PROPERTY_PREFIX){
					self.snap.funcs[uid].properties[prop] = makeSerializable(self.funcs[uid][prop], self, serializedObjects);
				}
			});

		captureObject(timestamp, self.funcs[uid], self, capturedObjects);
	});

	// Object.keys(this.objs).forEach(function(uid){
	// 	self.snap.objs[uid] = makeSerializable(self.objs[uid]);
	// });

	Object.keys(refs).forEach(function(name){
		// console.log(name);
		self.snap.refs[name] = makeSerializable(refs[name], self, serializedObjects);
		( DEBUG && console.log(helpers.indent(self.level*4)+chalk.cyan('var ')+name+':') );
		captureObject(timestamp, refs[name], self, capturedObjects);
		// self.snap.refs[name] = makeSerializable(refs[name], capturedObjects);
	});

	// This condition is true only in RootScope
	if (this.timers){
		Object.values(this.timers).forEach(function(timer){
			timer.callback[PROPERTY_PREFIX+'owner'].capture(timestamp);
		})
	}

	if (this.parent){
		// this.parent.children[this.uid] = this;
		this.parent.capture(timestamp);
		this.parent.snap.children[this.uid] = this.snap;
		this.parent.snap.depth = 1 + Object.values(this.parent.snap.children).reduce((acc, child)=> (child.depth > acc ? child.depth : acc), 1);
		( DEBUG && console.log(helpers.indent(this.level*4)+chalk.blue("added Snapshot ")+this.uri()+chalk.blue(' on parent ')+ this.parent.uri()) );
	}

	( DEBUG && console.log(helpers.indent(this.level*4)+chalk.green("Captured ")+this.uri()) );
	// this.CAPTURING = 2;
	return this.snap;
}
...
Scope.generateCode = function(obj, parent_identifier, scope_uid){
	...
	.sort((a, b)=> (obj.children[a].depth - obj.children[b].depth))
	.forEach(function(child_uid){
	...
	if (parent_identifier){
		...
		template += 'var '+identifier+' = new '+SCOPE_PREFIX+'.Scope(this, '+parent_identifier+', '+obj.name+', function(){ return [{'+params+'}, {'+refs+'}] }, "'+scope_uid+'").restore('+parent_identifier+')'+obj.hoisted.map(function(key){ return '.hoist('+key+', '+obj.funcs[key].context+')' }).join('') + ';';
		template += code;
	...
	// if root scope
		...
		code = identifier+'.setExtractor(function(){ return [{}, {'+refs+'}] })'+obj.hoisted.map(function(key){ return '.hoist('+key+', '+obj.funcs[key].context+')' }).join('') + ';' + code;
	}

	return code;
}

function Pointer(uri, rootScope){
	this.uri = uri;
	this.rootScope = rootScope;
}
Pointer.prototype.link = function(){
	console.log('Linking object '+this.uri);
	let obj = this.rootScope.getObject(this.uri);
	console.log(obj);
	return this.rootScope.getObject(this.uri);
}

var LastTimerId = 0;
function Timer(callback, timedelta, timer_id){
	this.id = (timer_id || NodeProcessId + (LastTimerId++));
...
ImmediateTimer.prototype.__callback = function(self){
	self.callback();
	self.cleared_at = Date.now();
	(self.onClear && self.onClear());
}
ImmediateTimer.prototype.start = function(){
	...
	// 	(self.onClear && self.onClear());
	// });
	this.ref = setImmediate(this.__callback, this);
};
...
var MasterIPCHandler = {
	...
	'stats': function(proc, payload){
		( DEBUG && console.log('\t'+payload.timestamp+' | '+chalk.yellow(payload.cpu.toFixed(2)+' %\t'+(payload.memory/1e6).toFixed(2)+' MB')) );
...
function Process(code, options, history){
	...
	self.code = code;
	self.obj = child_process.fork(path.join(__dirname, 'vm.js'), ['--stack-size=8192'], {
		silent: (self.options.silent || !!self.options.onStdout) 
	});
	...
	this.obj.on('message', function(message){
		...
		if (message.reply_id in self._requests){
			...
			console.log(chalk.red('Process '+self.code.name+' ERROR!'), message.error);
...
Process.prototype.saveLastSnapshot = function(file_path){
	if (this.snapshots.length > 0){
		...
		fs.writeFile(file_path, JSON.stringify(this.snapshots[this.snapshots.length-1].snapshot), function(err){
			if (err) throw err;
		})
	}	
}
...
var RuntimeHandler = {
	...
	SNAPSHOT: function(root_scope, kwargs){
		...
		console.log(chalk.magenta('Child Process sending Snapshot back!'));
...
function RootScope(local_module, pubsub_url, code_uri, user_meta){
	...
	this.meta = {
		...
		instance_id: (identity[1] || (identity[0] + "." + LastComponentId++)),
		extra: user_meta
	};
	this.timers = {};
	// this.root = undefined;
...
RootScope.prototype.snapshot = function(){
	...
	var scope_tree = this.capture(snapshot.meta.timestamp);
...
RootScope.prototype.getObject = function(uri){
	var tokens = uri.split('.');
	var scope = this.getScope(tokens[0].split('/').slice(1));
	console.log('Getting Object '+uri, scope);
	if (scope) return scope.objs[tokens[1]];
	throw "No Scope found for "+uri;
}
RootScope.prototype.addObject = function(obj, obj_id){
	var uid = obj_id.split('.')[1];
	var scope = this.getScope(obj_id.split('.')[0].split('/').slice(1));
...
RootScope.prototype.restoreObject = function(obj, proto_uri, scope_uri, obj_id){
	// var proto = this.getRawFunction(proto_uri).prototype;
	var scope = this.getScope(scope_uri.split('/').slice(1));
	if (!scope) throw "No Scope found for "+scope_uri;

	var oScope = this.getScope(obj_id.split('.')[0].split('/').slice(1));
	if (!oScope) throw "No Scope found for "+obj_id;
	var oUid = obj_id.split('.')[1];

	var proto = this.getFunction(proto_uri).prototype;
	var created = Object.create(proto);
	created[PROPERTY_PREFIX+'scope'] = scope;
	// created[PROPERTY_PREFIX+'uid'] = uid;
	// created[PROPERTY_PREFIX+'owner'] = this;
	oScope.objs[oUid] = created;
	console.log('Added object '+oUid+' to scope '+oScope.uri());
	Object.keys(obj).forEach(function(prop){
		if (obj[prop] instanceof Pointer) created[prop] = obj[prop].link();
		else created[prop] = obj[prop];
	});
...
RootScope.prototype.createPointer = function(uri){
	return new Pointer(uri, this);
}
