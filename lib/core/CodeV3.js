...
Code.prototype.run = function(options){
	var self = this;
	return new Promise(function(resolve, reject){
		var proc = new Process(self, options);
...
Code.prototype.kill = function(){
	Object.values(this.processes).forEach(function(proc){
		proc.kill();
	})
}
...
Scope.prototype.capture = function(params, refs){
	...
	Object.keys(params).forEach(function(name){
		...
		if (typeof params[name] === 'function'){
			params[name][PROPERTY_PREFIX+'owner'].extractor();
		}
	});

	Object.keys(this.funcs).forEach(function(uid){
		// if (refs[name][PROPERTY_PREFIX+'owner'] === self){
			// var func_uid = refs[name][PROPERTY_PREFIX+'uid'];
		self.snap.funcs[uid] = {
			type: 'Function',
		...
		// self.funcs[uid][PROPERTY_PREFIX+'owner'].extractor();
	});
	...
	Object.keys(refs).forEach(function(name){
		...
		if (typeof refs[name] === 'function'
			&& refs[name][PROPERTY_PREFIX+'owner']
			&& refs[name][PROPERTY_PREFIX+'owner'] !== self){

			refs[name][PROPERTY_PREFIX+'owner'].extractor();

		}
		else if (refs[name] && refs[name][PROPERTY_PREFIX+'scope']){
			refs[name][PROPERTY_PREFIX+'scope'].extractor();
		}
...
function Process(code, options){
	...
	// Initialize runtime
	this.obj.send({ code: self.code.source, options: { enableStats: this.options.enableStats } }, function(){
		started = Date.now();
	});
	
	this.obj.on('close', function(exit_code, signal){
		console.log(chalk.red('Process '+self.code.name+' stdio closed ')+exit_code);
	});
	...
	this.obj.on('exit', function(exit_code, signal){
		...
		console.log(chalk.red('Code '+self.code.name+' exited ')+exit_code+ ' --- '+(ended-started)+'ms');
	...
	this.on('status-update', function(){
		self.reportStatus();
	})
}
...
Process.prototype.reportStatus = function(){
