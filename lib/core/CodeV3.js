...
Code.instrument = function(pubsub, code_name, raw_code){
	...
	var template_str = `require('things-js').bootstrap(function(${SCOPE_PREFIX}){}, '${pubsub.url}', '${code_name}', ${meta});`;
	...
	template.body[0].expression.arguments[0].body.body = ast.body;
...
Code.fromSnapshot = function(snapshot, dummy_pubsub){
	...
	code.source = `require('things-js').bootstrap(function(${SCOPE_PREFIX}){ ${content} }, '${code.pubsub.url}', '${snapshot.meta.code_name}/${snapshot.meta.instance_id}', ${meta});`;
...
function extractObject(obj, scope){
	if (obj){
		if (typeof obj === 'function' && obj[PROPERTY_PREFIX+'owner'] !== scope){
		...
		else if (typeof obj === 'object' && obj[PROPERTY_PREFIX+'scope']){
...
Code.bootstrap = function(main, pubsub_url, code_uri, user_meta){
	return main(new RootScope(pubsub_url, code_uri, user_meta));
}
