  ...
  this.localplanCallback=function(data,topic){
    if(topic=='THINGS/LOADBALANCE'){
      ...
      if(self.reconfiguratior.size>0){
          self.reconfigure(self.reconfiguratior).then(function(){
...
PubsubEdge.prototype.reconfigure=function(reconfiguratior){
  var self=this
  multilogger.log("["+self.devicename+"] Reconfigure plan :"+JSON.stringify([...reconfiguratior]));
  var reconfiguring=function(address,topic){
    return new Promise(function(resolve,reject){
      if(self.topicCallbackHandler.has(topic)&& self.connectionStack.has(self.localPlan.get(topic))){
        ...
        if(self.connectionStack.has(address)){
          ...
          var connObj= new PubsubComponent(clientName,address,(new Map()).set(topic,topicCallbackHandler.get(topic)),true,self.removeConnection,self.connectionStack)
...
//   reconfiguratior.forEach(function(address,topic){
//     if(self.connectionStack.has(self.localPlan.get(topic))){
//       self.connectionStack.get(self.localPlan.get(topic)).unsubscribe(topic)// Unsubscribing the current connection
//     }
...
// })
  ...
  // self.connectionStack.forEach(function(connObj,stackKey){
  //   connObj.reconfigure(reconfiguratior,self.topicCallbackHandler)
  //   // if(self.connectionStack.has(address)){ //When reconfiguration has some changes on existing connections
  //   //   self.connectionStack.get(address).reconfigure(topic,address,self.topicCallbackHandler)
  //   // }else{
