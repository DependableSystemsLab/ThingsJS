...
function PubsubComponent(name, pubsubUrl, subscriptions, connectImmediately,connFailcallback,connectionStack){
    ...
    if (connectImmediately){
    	this.connect(function(ok){
			console.log('connected to mini-broker : '+self.pubsub_url)
		}).catch(function(error){
			console.log('Failed to connect mini-broker : '+self.pubsub_url)
			//To remove the connection from connectionStack if connection is failed
			if(connFailcallback){
...
//	        f();
			...
			self.handlers.forEach(function(callback,topic){
				self.client.subscribe(topic,function(){
					console.log('['+topic+']'+' connected to '+self.pubsub_url)
				})
			});
	    });
	    ...
	    self.client.on('message', function (topic,message) {
	    	try {
				...
				console.log("Message from  :"+self.clientName+" with topic "+topic+" "+d)
	    	}
	        catch (e){
	        	d = message;
	        }
			...
			console.log("Dispatching message to "+topic);
...
PubsubComponent.prototype.reconfigure=function(reconfigurator,topicCallbackHandler){
	...
	reconfigurator.forEach(function(address,topic){
		// CASE: change in plan, remove from this connection
		if(self.handlers.has(topic) && self.pubsub_url!=address){
			...
			console.log("Reconfiguration:Removing topic : "+topic+" from "+self.pubsub_url+' connection')
		}
		//CASE: change in plan, add topic to this connection
		if(!self.handlers.has(topic)&& self.pubsub_url==address){
			self.subscribe(topic,topicCallbackHandler.get(topic))
			console.log("Reconfiguration:Adding topic : "+topic+" to "+self.pubsub_url+' connection')
		}
