...
function parseDifference(diff){
	var cmds = ['run', 'stop', 'continueRun', 'migrate'];
	var parsedDiff = {};
	...
	function parseGeneral(diff, action, hashmap){
		diff[action].forEach(function(cmd){
			var device = cmd.device;
			var component = cmd.app;
			if(hashmap[action][device]){
				hashmap[action][device].push(component);
			}
			else{
				hashmap[action][device] = [component];
			}
		});
	}

	for(var i = 0; i < cmds.length - 1; i++){
		parseGeneral(diff, cmds[i], parsedDiff);
	}

	diff.migrate.forEach(function(migrateCmd){
		var fromDevice = migrateCmd.from;
		var toDevice = migrateCmd.to;
		var component = migrateCmd.app;
		parsedDiff['migrate'][component] = { from: fromDevice, to: toDevice };
	});
