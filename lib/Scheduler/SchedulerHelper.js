...
/**
 * @returns {Object[]} A list of devices and their latest memory reading
 */
SchedulerHelper.prototype.getDevices = function(){
	var self = this;
	var devices = [];

	for(engineId in this.dispatcher.engines){
		var engine = this.dispatcher.engines[engineId];
		var latestStat = engine.stats[engine.stats.length-1];
		var meta = { device: engine.id, memory: latestStat['memory'] };
		devices.push(meta);
	}
	return devices;
}
...
/**
 * Get the memory stats for a specific device
 */
SchedulerHelper.prototype.getDeviceMemory = function(device){
	var self = this;
	if(device in self.dispatcher.engines){
		var stats = self.dispatcher.engines[device].stats;
		var latestStat = stats[stats.length-1];
		return latestStat['memory'];
	}
	else return {};
}
