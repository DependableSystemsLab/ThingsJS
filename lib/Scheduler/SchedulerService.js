	...
	return new Promise(function(resolve, reject){
		...
		if(self.currentSchedule){
			...
			if((diff['run'].length | diff['stop'].length | diff['migrate'].length) === 0){
				 ...
				 * both schedules have the same key length (-1 for the timestamp)
				   @@ HACKY CHECK
				 */
 ...
 SchedulerService.prototype.appendToHistory = function(schedule){
 	...
 	return new Promise(function(resolve, reject){
		...
		Promise.all([p1, p2]).then(function(){
			/* notify user of schedule update to filesystem, and the operations applied
			 * TODO: optimize later because we already compute the difference once.
             */
 			var operations = ComputeDifference(schedule, self.currentSchedule || {}); 
			self.pubsub.publish(SCHEDULE_UPDATE_TOPIC, { schedule: schedule, operations: operations });
