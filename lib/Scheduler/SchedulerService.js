...
function SchedulerService(){
	...
	this.startScheduling = function(){
		return new Promise(function(resolve){
			if(self.inSchedule){
				console.log('\x1b[44m%s\x1b[0m', '[Scheduler Service] Waiting for current scheduling period to finish...')
				self.waitTask('done schedule').then(function(){
...
SchedulerService.prototype.runApplicationTrigger = function(){
	...
	self.pubsub.subscribe(RUN_APPLICATION_NAMESPACE, function(runData){
		...
		var replyData = { status: 'PENDING', application_id: appId, name: runData.name };
