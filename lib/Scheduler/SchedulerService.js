...
SchedulerService.prototype.runApplicationTrigger = function(){
	...
	self.pubsub.subscribe(RUN_APPLICATION_NAMESPACE, function(runData){
		...
		self.schedule({ RUN: true }).then(function(instanceData){
			...
			self.pubsub.publish(REPLY_APPLICATION_NAMESPACE+'/'+runData.id+'/run', { id: runData.id, instances: instanceData });
			self.run();
		})
		.catch(function(err){
			self.run();
			console.log('[Scheduler Service] Failed to run application '+ runData.id + '. Error: '+err);
			self.pubsub.publish(REPLY_APPLICATION_NAMESPACE+'/'+runData.id+'/run', { id: runData.id, instances: [] });
		});
	});
}
	...
	self.pubsub.subscribe(STOP_APPLICATION_NAMESPACE, function(stopData){
		...
		stopComponents.forEach(function(component){
			...
			self.schedule({ STOP: true }).then(function(data){
				...
				self.pubsub.publish(REPLY_APPLICATION_NAMESPACE+'/'+appId+'/stop', { id: appId, status: true });
				self.run();
			})
			.catch(function(err){
				console.log('[Scheduler Service] Failed to stop an application '+err);
				self.pubsub.publish(REPLY_APPLICATION_NAMESPACE+'/'+appId+'/stop', { id: appId, status: false });
				self.run();
			});
		});
	...
	return new Promise(function(resolve, reject){
		...
		if(self.currentSchedule){
			/* if a current schedule exists, we compute the difference between two schedules.
			   If a schedule is undefined, we compute an empty schedule because a user may
			   want to stop all components
			 */
			...
			if((diff['run'].length | diff['stop'].length | diff['migrate'].length) === 0){
				...
				/* if there is no diff but the key length has changed, we have new devices.
				   We subtract 1 for the timestamp field

				   @@ CURRENTLY HACKY CHECK
				 */
				if(Object.keys(schedule).length !== (Object.keys(self.currentSchedule).length -1)){
			...
			// a current schedule does not exist, run without computing difference
			if(!schedule){
				reject(new Error('No schedule to run'));
				return;
			}
...
SchedulerService.prototype.clearRecords = function(){
	...
	function reAddFolders(){
		...
		return Promise.all([history, current]).then(function(){
 ...
 * and notifying listeners of the change over pubsub
 *
 ...
 */
 SchedulerService.prototype.appendToHistory = function(schedule){
 	...
 	// append a timestamp to the schedule
 	...
 	return new Promise(function(resolve, reject){
		...
		Promise.all([p1, p2]).then(function(){
			// notify user of schedule update to filesystem
			...
			// we don't need to keep track of the timestamp on our end
		...
		for(device in self.devicesLog){
			...
			*/
			...
			if(self.currentSchedule && self.currentSchedule[device]){
				runningComponents = Object.keys(self.currentSchedule[device]);
			}
			...
			var componentMemory = runningComponents.reduce(function(sum, compId){
				var stat = self.componentsLog[compId] ? self.componentsLog[compId].slice(-1).pop() : { heapUsed : 0 };
