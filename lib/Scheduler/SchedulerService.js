...
// 		var replyToken = runData.reply_token;
...
// 		var replyData = { id: appId, name: runData.name, replyTo: replyToken, instances: [] };
// 		// invoke a rescheduling
// 		self.schedule({ RUN: true }).then(function(instanceData){
// 			console.log('[Scheduler Service] Replying to user with instance details on '+REPLY_APPLICATION_NAMESPACE);
...
// 			self.pubsub.publish(REPLY_APPLICATION_NAMESPACE, { id: appId, name: runData.name, instances: [] });
// 			self.run();
// 		});
...
SchedulerService.prototype.stopApplicationTrigger = function(){
	...
	self.pubsub.subscribe(STOP_APPLICATION_NAMESPACE, function(stopData){
		...
		var replyToken = stopData.reply_token;
		...
		// invoke rescheduling
		var replyData = { replyTo: replyToken, status: false };
		self.schedule({ STOP: true }).then(function(data){
			console.log('[Scheduler Service] Replying to STOP request on '+REPLY_APPLICATION_NAMESPACE);
	...
	return new Promise(function(resolve, reject){
		Promise.all([devices, comps]).then(function(data){
			...
			self.apply(schedule).then(function(res){
				...
				// cache current mapping and schedule
				self.currentMapping = schedule;
				self.currentSchedule = formattedSchedule;
