	...
	return new Promise(function(resolve, reject){
		...
		if(self.currentSchedule){
			...
			self.helper.reSchedule(diff).then(function(data){
				/* notify user that a schedule has been applied, and its operations
                   TODO: optimize
                 */
				var operations = ParseDifference(diff, self.helper.componentMappings);
			...
			self.helper.runSchedule(schedule).then(function(data){
				self.pubsub.publish(SCHEDULE_UPDATE_TOPIC, { schedule: schedule });
				resolve(data);
			})
			.catch(function(err){
