...
SchedulerService.prototype.clearRecords = function(){
	...
	return new Promise(function(resolve, reject){
		var p0 = self.fsHelper.checkDirExists(self.fsHelper.cur_path);
		var p1 = self.fsHelper.checkDirExists('history');
		var p2 = self.fsHelper.checkDirExists('current');

		Promise.all([p0, p1, p2]).then(function(exists){
			var p3, p4;
			var scheduleExists = (exists[0]) ? true : false;
			...
			Promise.all([p3, p4]).then(function(){
				...
				if(!scheduleExists){
					...
					createdSchedule = self.fsHelper.makeDir('schedule');
				}
				Promise.all([createdSchedule]).then(function(result){
					self.fsHelper.navigateTo('schedule', true);
					...
					.catch(function(err){
						console.log('[Scheduler Service] Failed to make empty HISTORY/CURRENT folders: '+err);
						reject(err);
					});
				})
				.catch(function(err){
					console.log('[Scheduler Service] Could not create a schedule folder');
					reject(err);
				});
				// var p5 = self.fsHelper.makeDir('current');
				// var p6 = self.fsHelper.makeDir('history');
