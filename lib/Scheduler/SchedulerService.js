...
// SchedulerService specific variables
...
SchedulerService.prototype.runApplicationTrigger = function(){
	...
	self.pubsub.subscribe(RUN_APPLICATION_NAMESPACE, function(runData){
		...
		var appId = randKey();
		var app = new Application(appId, Application.status.PENDING);
		...
		var replyData = { status: Application.status.PENDING, application_id: appId, name: runData.name };
		var statusSent = false;

		/* invoke a rescheduling with a RUN flag to indicate we want to run
		 * components indicated in this.componentsToRun
		 */
		if(self.inSchedule){
...
SchedulerService.prototype.stopApplicationTrigger = function(){
	...
	self.pubsub.subscribe(STOP_APPLICATION_NAMESPACE, function(stopData){
		...
		var replyData = { application_id: appId, status: Application.status.STOP_FAILED };
		 ...
		 * remove all components in this.componentsToStop 
		 */
		...
		.catch(function(err){
			...
			self.pubsub.publish(replyTopic, replyData);
			self.application[appId].status = Application.status.STOP_FAILED;
			self.finishedScheduling();
			self.run();
		});
	...
	return new Promise(function(resolve, reject){
		...
		self.waitTask('device adjust').then(function(){
			...
			deadComponents.forEach(function(compId){
				...
				for(device in self.currentSchedule.mapping){
