...
var SchedulerHelper = require('./mod_SchedulerHelper.js');
...
SchedulerService.prototype.apply = function(schedule){
	...
	return new Promise(function(resolve, reject){
		...
		self.helper.reSchedule(diff).then(function(data){
			...
			data[true].forEach(function(op){
				var operation = op.op;
				var result = op.result;

				if(returnData[operation.component]){
					returnData[operation.component].push(result.id);
				}
				else{
					returnData[operation.component] = [result.id];
				}
			})
			resolve(returnData);			
		})
		.catch(function(errData){
			console.log('[Scheduler Service] Error running schedule! Trying to fix...');
			// we must reject in both cases here so the computed schedule is not appended
			self.fixSchedule(errData).then(function(){
				reject(new Error('Computed schedule could not be applied'));
			})
			.catch(function(){
				reject(new Error('Computed schedule could not be applied'));
			});
...
SchedulerService.prototype.fixSchedule = function(errLog){
	...
	// apply all successful operations to the schedule for now
	errLog[true].forEach(function(successOp){
		...
		switch(operation.cmd){
			case 'run':
				if(updatedSchedule[operation.device]){
					updatedSchedule[operation.device].push(operation.component);
				}
				else{
					updatedSchedule[operation.device] = [operation.component];
				}
				break;
			case 'kill':
				...
				var index = updatedSchedule[device].indexOf(compId);
				updatedSchedule[device].splice(index, 1);
				break;
			case 'migrate':
				var compId = operations.component;
				var from = operations.from;
				var to = operations.to;

				var index = updatedSchedule[from].indexOf(compId);
				updatedSchedule[from].splice(index, 1);

				if(updatedSchedule[to]){
					updatedSchedule[to].push(compId);
				}
				else{
					updatedSchedule[to] = [compId];
				}
				break;
		}
	});
	console.log('[Scheduler Service] Repaired current schedule');
