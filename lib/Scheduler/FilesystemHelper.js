...
FilesystemHelper.prototype.checkDirExists = function(dir_name){
	...
	return rp(options)
		.then(function(res){
			console.log('[Filesystem Helper] GET Dir status: '+dir_name +res.statusCode);
			console.log('[Filesystem Helper] GET Dir body: '+dir_name + res.body.toString());
...
FilesystemHelper.prototype.writeFile = function(target_path, file_data, file_name){
	...
	return new Promise(function(resolve, reject){
		...
		self.getFile(self.cur_path + '/' + file_name).then(function(data){
...
FilesystemHelper.prototype.deleteTarget = function(target){
	...
	return new Promise(function(resolve, reject){
		...
		self.getFile(self.cur_path + '/' + target).then(function(data){
			...
			rp(options)
				.then(function(parsed_body){
					console.log('[Filesystem helper] DELETE target success: ' + target + parsed_body);
					resolve();
				})
				.catch(function(err){
