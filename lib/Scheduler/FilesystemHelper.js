...
const FS_URL_PATH = 'http://localhost:3000/fs';
...
FilesystemHelper.prototype.joinPath = function(p1, p2){
	...
	this.log("[Filesystem Helper] Joined path: " +p1+'/'+p2);
...
FilesystemHelper.prototype.navigateTo = function(dir_name, reset){
	...
	this.log('[Filesystem Helper] Current path: ' + self.cur_path);
}
...
FilesystemHelper.prototype.checkDirExists = function(dir_name){
	...
	return rp(options)
		...
		.catch(function(err){
			self.log('[Filesystem Helper] GET Dir error: '+err);
			return false;
		});
}
	...
	return new Promise(function(resolve, reject){
		rp(options)
			.then(function(res_body){
				...
				self.log('[Filesystem Helper] GET FILE success '+ file_path);
				resolve(res_body);
			})
			.catch(function(err){
				self.log('[Filesystem Helper] GET FILE error '+ err);
				reject(err);
			});
	});
	...
	return new Promise(function(resolve, reject){
		rp(options)
			.then(function(parsed_body){
				self.log('[Filesystem Helper] makeDir success '+ parsed_body);
				resolve();
			})
			.catch(function(err){
				self.log('[Filesystem Helper] makeDir error '+ err);
				reject(err);
			});
	});
}
...
FilesystemHelper.prototype.writeFile = function(target_path, file_data, file_name){
	...
	function post(options, resolve, reject){
		rp(options).then(function(parsed_body){
			self.log('[Filesystem Helper] writeNewFile succes: '+file_name);
			resolve();
		})
		.catch(function(err){
			self.log('[Filesystem Helper] writeNewFile error: '+file_name);
			reject(err);
		});
	}
	...
	return new Promise(function(resolve, reject){
		...
		self.getFile(target_path + '/' + file_name).then(function(data){
			...
			self.log('[Filesystem Helper] '+file_name+ ' exists with _id '+data._id);
			post(options, resolve, reject);
		})
		.catch(function(err){
			self.log('[Filesystem Helper] '+file_name+ ' will be created');
			post(options, resolve, reject);
		});
...
FilesystemHelper.prototype.deleteTarget = function(target){
	...
	return new Promise(function(resolve, reject){
		...
		self.getFile(self.cur_path + '/' + target).then(function(data){
			id = data._id;
			self.log('[Filesystem Helper] _id to be deleted: '+id);
			...
			rp(options)
				.then(function(parsed_body){
					self.log('[Filesystem helper] DELETE target success: ' + target);
					resolve();
				})
				.catch(function(err){
					self.log('[Filesystem helper] DELETE target error: ' + target + err);
					reject(err);
				});
		})
		.catch(function(err){
			self.log('[Filesystem Helper] getFile error: '+err);
			reject(err);
		});
	});
