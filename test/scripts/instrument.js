...
CODES.forEach(function(code_name){
	...
	var raw = fs.readFileSync(path.join(BASE_DIR, code_name+'.js')).toString();
...
output.push( 'Raw Size\t'+ CODES.map(function(code_name){ return result[code_name].size_before }).join('\t') );
output.push( 'Instrumented Size\t'+ CODES.map(function(code_name){ return result[code_name].size_after }).join('\t') );
...
fs.writeFile(path.join(RESULT_DIR, 'instrument.'+SESSION+'.csv'), output, function(err){
	if (err) throw err;
