...
describe('Single instance', function(){
	...
	it('has an assigned id', function(){
		expect(engine.id).to.be.a('string');
	});

	it('has an assigned pubsub object', function(){
		expect(engine.pubsub).to.be.an('object');
	});

	it('runs code', function(done){
		this.timeout(5000);
		engine.run_code({ code_name: 'count.js', source: 'var c=0;setInterval(function(){ console.log(c++); }, 100)' })
	...
	it('dies gracefully on .kill()', function(done){
		engine.kill()
		.then(function(){ done() });
	})
});

// describe('CodeEngine Test', function(){
// 	var self = this;
...
// 	// 	self.codeEngines[i] = fork(path.resolve(__dirname, './helpers/forkengine.js'),
...
// 		// before(function(){
// 		// 	this.timeout(5000);
// 		// 	var config = DEFAULT_ENGINE;

// 		// 	for(var i = 0; i < NUMFORKS; i++){
// 		// 		self.codeEngines[i] = new things.CodeEngine(config, { logging: 'thingsjs-logs', logInterval: 500 })
// 		// 	}
...
// 			var args = { code_name: 'count.js', source: 'var c=0;setInterval(function(){ console.log(c++); }, 100)'};
// 			self.codeEngines[0].send({ cmd: 'run', args: args });
// 			// self.codeEngines[0].run_code({ code_name: 'count.js', source: 'var c=0;setInterval(function(){ console.log(c++); }, 100)' });
...
// 			var args = { code_name: 'negCount.js', source: 'var c=0;setInterval(function(){ console.log(c--); }, 100)'};
// 			self.codeEngines[0].send({ cmd: 'run', args: args });
