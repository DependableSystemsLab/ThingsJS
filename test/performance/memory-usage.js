...
function serialExecute(code, count){
	...
	return new Promise(function(resolve, reject){
		...
		var timer = setInterval(function(){
			code.getStats().then(function(stats){
				mems.push(stats.memory.heapUsed);
				console.log(chalk.yellow('Memory : ')+stats.memory.heapUsed)
			})
		}, 1000);
		code.once('run-finished', function(){
			clearInterval(timer);
...
.then(function(){
	...
	fs.writeFile(path.join(__dirname, 'result/memory-usage.'+testSession+'.csv'), output, function(err){
		if (err) throw err;
